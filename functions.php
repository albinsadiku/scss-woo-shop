<?php
/**
 * teeflex functions and definitions
 *
...
if ( ! function_exists( 'teeflex_setup' ) ) :
	/**
	 ...
	 */
	function teeflex_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on teeflex, use a find and replace
		 * to change 'teeflex' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'teeflex', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'teeflex' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'teeflex_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'teeflex_setup' );
 ...
 */
function teeflex_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'teeflex_content_width', 640 );
}
add_action( 'after_setup_theme', 'teeflex_content_width', 0 );
 ...
 */
function teeflex_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'teeflex' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'teeflex' ),
...
add_action( 'widgets_init', 'teeflex_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function teeflex_scripts() {
	wp_enqueue_style( 'teeflex-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_enqueue_style( 'teeflex-main', get_template_directory_uri() . '/css/main.css');
	wp_enqueue_style( 'bootstrap-icons', 'https://cdn.jsdelivr.net/npm/bootstrap-icons@1.4.0/font/bootstrap-icons.css');
	wp_style_add_data( 'teeflex-style', 'rtl', 'replace' );
	
	wp_enqueue_script( 'teeflex-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
	...
	wp_enqueue_script( 'teeflex-script', get_template_directory_uri() . '/js/script.js', array('jquery') );
...
add_action( 'wp_enqueue_scripts', 'teeflex_scripts' );
...
function enqueue_custom_fonts() {
	if(!is_admin()) {
		wp_register_style('Montserrat', 'https://fonts.googleapis.com/css2?family=Montserrat:wght@100&display=swap');
		wp_register_style('open_sans', 'https://fonts.googleapis.com/css2?family=Nunito:wght@400;700&display=swap');
